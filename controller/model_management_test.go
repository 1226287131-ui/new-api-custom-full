...
func modelManagementDB(t *testing.T, kind, dsn string) *gorm.DB {
	...
	// InitDB opens the isolated database through the production startup path.
	// Release the fixture's first connection before replacing its handle.
	initialConnection, err := database.DB()
	...
	restoreRatios := []struct {
		...
		{ratio_setting.ImageResolutionPrice2JSONString(), ratio_setting.UpdateImageResolutionPriceByJSONString},
	...
	config.UpdateConfigFromMap(config.GlobalConfig.Get("billing_setting"), map[string]string{"billing_mode": "{}", "billing_expr": "{}", "plugin_billing_expr": "{}", "task_billing_pricing": "{}", "scheduled_discount": "{}"})
	var version string
	...
	t.Cleanup(func() {
		...
		config.UpdateConfigFromMap(config.GlobalConfig.Get("billing_setting"), map[string]string{"billing_mode": previousConfig["billing_setting.billing_mode"], "billing_expr": previousConfig["billing_setting.billing_expr"], "plugin_billing_expr": previousConfig[billing_setting.PluginBillingExprOption], "task_billing_pricing": previousConfig["billing_setting.task_billing_pricing"], "scheduled_discount": previousConfig["billing_setting.scheduled_discount"]})
