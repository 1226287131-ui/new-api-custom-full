...
func valuesEqual(a, b any) bool {
	...
	left, leftErr := common.Marshal(a)
	right, rightErr := common.Marshal(b)
...
var pricingSyncFields = []string{
	...
	billing_setting.ScheduledDiscountField,
}
...
func effectivePricingSyncData(data map[string]any) map[string]any {
	...
	for name := range names {
		...
		if modes[name] == billing_setting.BillingModeTieredExpr {
			...
			delete(valueMap(result["image_resolution_price"]), name)
			delete(valueMap(result[billing_setting.TaskBillingPricingField]), name)
			continue
		}
		delete(expressions, name)
		_, imageResolution := valueMap(result["image_resolution_price"])[name]
		taskPricing, task := valueMap(result[billing_setting.TaskBillingPricingField])[name]
		if imageResolution {
			modes[name] = "image_resolution"
		} else if task {
			modes[name] = valueMap(taskPricing)["mode"]
		} else if modes[name] != billing_setting.BillingModePerRequest && modes[name] != billing_setting.BillingModePerSecond {
		...
		if !fixed && !token && !imageResolution && !task {
			for _, field := range pricingSyncFields {
				delete(valueMap(result[field]), name)
			}
			continue
		}
		if fixed || imageResolution || task {
			for field := range numericPricingSyncFields {
				if field != "model_price" || imageResolution || task {
					delete(valueMap(result[field]), name)
				}
			}
		}
...
func FetchUpstreamRatios(c *gin.Context) {
	...
	for _, chn := range upstreams {
		...
		go func(chItem dto.UpstreamDTO) {
			...
			var pricingItems []struct {
				...
				ScheduledDiscount     *billing_setting.ScheduledDiscountConfig `json:"scheduled_discount"`
				ImageResolutionPrices *ratio_setting.ImageResolutionPrice      `json:"image_resolution_prices"`
			...
			imageResolutionPriceMap := make(map[string]ratio_setting.ImageResolutionPrice)
			taskBillingPricingMap := make(map[string]billing_setting.TaskBillingPriceConfig)
			scheduledDiscountMap := make(map[string]billing_setting.ScheduledDiscountConfig)
			...
			for _, item := range pricingItems {
				...
				if item.ScheduledDiscount != nil {
					scheduledDiscountMap[item.ModelName] = *item.ScheduledDiscount
				}
				if item.ImageResolutionPrices != nil {
					imageResolutionPriceMap[item.ModelName] = *item.ImageResolutionPrices
				}
				if item.BillingMode == billing_setting.BillingModeTieredExpr {
					continue
				}
			...
			if len(imageResolutionPriceMap) > 0 {
				converted["image_resolution_price"] = valueMap(imageResolutionPriceMap)
			}
			if len(scheduledDiscountMap) > 0 {
				converted[billing_setting.ScheduledDiscountField] = valueMap(scheduledDiscountMap)
			}
	...
	for name, fields := range differences {
		...
		for _, channel := range successfulChannels {
			...
			_, hasImagePrice := candidate["image_resolution_price"]
			_, hasTaskPrice := candidate[billing_setting.TaskBillingPricingField]
			if hasRatio || hasPrice || hasExpression || hasImagePrice || hasTaskPrice {
