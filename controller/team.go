...
const (
	securityProofScopeTeamPayoutWrite      = "team.payout.write"
	securityProofScopeTeamWithdrawalWrite  = "team.withdrawal.write"
	securityProofScopeTeamWithdrawalReview = "team.withdrawal.review"
	securityProofScopeTeamWithdrawalRead   = "team.withdrawal.read"
)
...
func SaveTeamPayoutAccount(c *gin.Context) {
	if !middleware.RequireSecurityProof(c, securityProofScopeTeamPayoutWrite, []string{"2fa", "passkey"}) {
		return
	}
	var request struct {
...
func RequestTeamWithdrawal(c *gin.Context) {
	if !middleware.RequireSecurityProof(c, securityProofScopeTeamWithdrawalWrite, []string{"2fa", "passkey"}) {
		return
	}
	var request struct {
...
func AdminReadTeamWithdrawal(c *gin.Context) {
	if !middleware.RequireSecurityProof(c, securityProofScopeTeamWithdrawalRead, []string{"2fa", "passkey"}) {
		return
	}
...
func AdminReviewTeamWithdrawal(c *gin.Context) {
	if !middleware.RequireSecurityProof(c, securityProofScopeTeamWithdrawalReview, []string{"2fa", "passkey"}) {
		return
	}
	id, err := strconv.ParseInt(c.Param("id"), 10, 64)
	if err != nil || id <= 0 {
