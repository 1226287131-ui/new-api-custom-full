...
func setupTeamProofTest(t *testing.T) (service.AuthIdentity, *model.AgentWithdrawal) {
	...
	require.NoError(t, db.AutoMigrate(&model.User{}, &model.Log{}, &model.AgentPolicy{},
	...
	return service.AuthIdentity{UserID: 42, SessionID: "proof-session", UserAuthVersion: 1, SessionVersion: 1}, withdrawal
}
...
func TestTeamSensitiveOperationsRejectInvalidProofWithoutMutationOrDisclosure(t *testing.T) {
	...
	for _, operation := range []struct {
		...
		t.Run(operation.name, func(t *testing.T) {
			...
			wrongSession, _, err := service.IssueSecurityProof(otherSession, "2fa", []string{operation.scope})
			require.NoError(t, err)
			wrongMethod, _, err := service.IssueSecurityProof(identity, "password", []string{operation.scope})
			require.NoError(t, err)
...
func TestTeamPayoutDisclosureRequiresCorrectSessionAndReadScope(t *testing.T) {
	identity, withdrawal := setupTeamProofTest(t)
	proof, _, err := service.IssueSecurityProof(identity, "passkey", []string{securityProofScopeTeamWithdrawalRead})
	require.NoError(t, err)
