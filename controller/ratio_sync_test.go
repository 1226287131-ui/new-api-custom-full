...
func TestPricingSyncPreservesCustomPriceStructures(t *testing.T) {
	discount := billing_setting.ScheduledDiscountConfig{Enabled: true, Start: "22:00", End: "02:00", Discount: 0.8}
	image := map[string]float64{"1K": 0, "2K": 0.2, "4K": 0.4}
	task := billing_setting.TaskBillingPriceConfig{Mode: "per-second", ResolutionPrices: map[string]float64{"1080p": 0.1}}
	...
	for _, tc := range []struct {
		...
		{
			name: "image tiers suppress stale scalar prices and preserve free tier",
			source: map[string]any{
				"image_resolution_price": map[string]map[string]float64{"m": image},
				"model_price":            map[string]float64{"m": 9},
				"model_ratio":            map[string]float64{"m": 3},
			},
			want: map[string]any{"billing_mode": "image_resolution", "image_resolution_price": map[string]any{"1K": 0.0, "2K": 0.2, "4K": 0.4}},
		},
		{
			name: "task price preserves mode resolution and discount",
			source: map[string]any{
				"task_billing_pricing": map[string]billing_setting.TaskBillingPriceConfig{"m": task},
				"scheduled_discount":   map[string]billing_setting.ScheduledDiscountConfig{"m": discount},
				"model_price":          map[string]float64{"m": 9},
			},
			want: map[string]any{
				"billing_mode":         "per-second",
				"task_billing_pricing": map[string]any{"mode": "per-second", "resolution_prices": map[string]any{"1080p": 0.1}},
				"scheduled_discount":   map[string]any{"enabled": true, "start": "22:00", "end": "02:00", "discount": 0.8},
			},
		},
		{
			name: "explicit expression keeps discount and suppresses inactive custom prices",
			source: map[string]any{
				"billing_mode":           map[string]string{"m": "tiered_expr"},
				"billing_expr":           map[string]string{"m": expression},
				"image_resolution_price": map[string]map[string]float64{"m": image},
				"task_billing_pricing":   map[string]billing_setting.TaskBillingPriceConfig{"m": task},
				"scheduled_discount":     map[string]billing_setting.ScheduledDiscountConfig{"m": discount},
			},
		...
		t.Run(tc.name, func(t *testing.T) {
			assert.Equal(t, tc.want, modelPricingSyncValues(effectivePricingSyncData(tc.source), "m"))
			encoded, err := common.Marshal(tc.source)
			require.NoError(t, err)
			var decoded map[string]any
			require.NoError(t, common.Unmarshal(encoded, &decoded))
			// Runtime configuration uses typed nested maps; upstream JSON uses
			// generic maps. Equal complete prices must produce no differences.
			assert.Empty(t, buildDifferences(tc.source, []struct {
				name string
				data map[string]any
			}{{name: "source", data: decoded}}))
		})
	}
}
