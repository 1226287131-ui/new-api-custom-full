...
func executeTaskSubmissionWith(
	...
	task.PrivateData.BillingContext = &model.TaskBillingContext{
		...
		PerCallBilling:    taskBillingModeForRelayInfo(relayInfo) == billing_setting.BillingModePerRequest,
		TieredSnapshot:    relayInfo.TieredBillingSnapshot,
	}
	task.Quota = result.Quota
	task.Data = result.TaskData
	if constant.IsVideoTaskPlatform(result.Platform) && !taskHasPluginExecution(task) {
...
func presentTaskSubmission(c *gin.Context, outcome *taskSubmissionOutcome) {
	...
	if outcome.Result.ClientResponse != nil {
		diagnostics.present(outcome.Task, "legacy_provider")
