...
type TaskBillingContext struct {
	...
	GroupRatio        float64            `json:"group_ratio"`                 // 分组倍率（允许显式为 0）
	ModelRatio        float64            `json:"model_ratio,omitempty"`        // 模型倍率
	OtherRatios       map[string]float64 `json:"other_ratios,omitempty"`       // 附加倍率（时长等）
	OriginModelName   string             `json:"origin_model_name,omitempty"`  // 模型名称，必须为OriginModelName
	BillingMode       string             `json:"billing_mode,omitempty"`       // 任务计费单位快照
	BillingResolution string             `json:"billing_resolution,omitempty"` // 选中的分辨率快照
