...
var modelPricingOptionKeys = []string{
	...
	"CreateCacheRatio", "ImageRatio", "ImageResolutionPrice", "ModelPrice", "ModelRatio",
	"billing_setting.billing_expr", "billing_setting.billing_mode", billing_setting.PluginBillingExprOption,
	"billing_setting.scheduled_discount", "billing_setting.task_billing_pricing",
}
...
func effectiveModelPricing(values map[string]map[string]any, name string) PricingValues {
	...
	for _, key := range []string{"ModelPrice", "ModelRatio", "CompletionRatio", "AudioRatio", "AudioCompletionRatio", "ImageResolutionPrice"} {
		delete(result, key)
	...
	if mode == "" {
		...
		_, hasImagePrice := result["ImageResolutionPrice"]
		_, hasTaskPrice := result["billing_setting.task_billing_pricing"]
		if _, builtin := billing_setting.GetBuiltinBillingExpr(name); builtin && !hasPrice && !hasRatio && !hasImagePrice && !hasTaskPrice {
			mode = "tiered_expr"
		}
	}
	if mode == "tiered_expr" {
	...
	if _, exists := result["ImageResolutionPrice"]; exists {
		return result
	}
	if _, exists := result["billing_setting.task_billing_pricing"]; exists {
		return result
	}
...
func validateModelPricing(name string, values, previous PricingValues) error {
	...
	for key, value := range values {
		...
		if key == "billing_setting.billing_mode" {
			if value != "ratio" && value != "tiered_expr" && value != billing_setting.BillingModePerRequest && value != billing_setting.BillingModePerSecond && value != "image_resolution" {
				return errors.New("invalid billing mode")
			}
			continue
		}
		if key == "ImageResolutionPrice" || key == "billing_setting.task_billing_pricing" || key == "billing_setting.scheduled_discount" {
			if fields, ok := value.(map[string]any); !ok || fields == nil {
				return fmt.Errorf("%s must be a pricing object", key)
			}
			...
			case "ImageResolutionPrice":
				err = ratio_setting.ValidateImageResolutionPriceJSONString(string(encoded))
			case "billing_setting.task_billing_pricing":
				err = billing_setting.ValidateTaskBillingPricingJSONString(string(encoded))
			case "billing_setting.scheduled_discount":
				err = billing_setting.ValidateScheduledDiscountJSONString(string(encoded))
			}
			if err != nil {
				return fmt.Errorf("%s: %w", key, err)
			}
