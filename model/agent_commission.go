...
func SnapshotEpayAgentPolicy(topUp *TopUp, price, quotaPerUnit float64) error {
	...
	if user.Quota > common.MaxQuota-quota || user.QuotaCreditTotal > maxUserQuotaCreditTotal-int64(quota) {
...
func calculateAgentCommission(snapshot agentOrderSnapshot, cents int64) (int, int64, error) {
	...
	quota, clamp := common.QuotaFromDecimalChecked(rebateCents.Div(decimal.NewFromInt(100)).Div(price).Mul(quotaPerUnit))
	if clamp != nil {
		return 0, 0, clamp
	}
	return quota, 0, nil
...
func epayTopUpQuota(amount int64, quotaPerUnit decimal.Decimal) (int, error) {
	...
	quota, clamp := common.QuotaFromDecimalChecked(decimal.NewFromInt(amount).Mul(quotaPerUnit).Truncate(0))
	if clamp != nil {
		return 0, clamp
	}
	if quota <= 0 {
...
func creditAgentQuota(tx *gorm.DB, userID, quota int) (int64, error) {
	...
	result := tx.Table("users").Where("id = ? AND deleted_at IS NULL AND quota <= ? AND quota_credit_total <= ?", userID, common.MaxQuota-quota, maxUserQuotaCreditTotal-int64(quota)).
