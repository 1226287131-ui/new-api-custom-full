...
func ResolveLegacyBillingDetails(name string, effective, configured PricingValues) LegacyBillingDetails {
	...
	if _, configured := effective["ImageResolutionPrice"]; configured {
		details.ImageCount = true
		return details
	}
	if _, configured := effective["billing_setting.task_billing_pricing"]; configured {
		return details
	}
...
func PreviewModelPricingConversion(name string, draft PricingValues) (*ModelPricingConversion, error) {
	...
	if _, configured := draft["ImageResolutionPrice"]; configured {
		return &ModelPricingConversion{UnsupportedReason: "Image resolution pricing must be converted manually while preserving every resolution tier and image count."}, nil
	}
	if _, configured := draft["billing_setting.task_billing_pricing"]; configured {
		return &ModelPricingConversion{UnsupportedReason: "Task resolution pricing must be converted manually using the task usage schema."}, nil
	}
	if mode := draft["billing_setting.billing_mode"]; mode == billing_setting.BillingModePerSecond || mode == billing_setting.BillingModePerRequest {
