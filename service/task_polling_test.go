...
func TestRedactVideoResponseBodyPreservesPollingPayloadShape(t *testing.T) {
	...
	require.NoError(t, common.Unmarshal(redactVideoResponseBody(body, "", false), &stored))
...
func (a *completedVideoPollingAdaptor) FetchTask(_ string, _ string, _ *model.Task, _ string) (*http.Response, error) {
	return &http.Response{
		StatusCode: http.StatusOK,
...
func (a *completedVideoPollingAdaptor) ParseTaskResult(*model.Task, *http.Response, []byte) (*relaycommon.TaskInfo, error) {
	result := a.result
	return &result, nil
}
...
func TestUpdateVideoSingleTaskKeepsCompletedStatusWhenCacheFails(t *testing.T) {
	truncate(t)
...
func TestUpdateVideoSingleTaskCacheFailureCannotRefundCompletedUpstreamTask(t *testing.T) {
	truncate(t)
	t.Setenv("VIDEO_CACHE_DIR", t.TempDir())
...
func TestUpdateVideoSingleTaskPollClassification(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			task.PrivateData.Execution = &model.TaskExecutionSnapshot{TaskPlugin: &model.TaskPluginSnapshot{Key: "kling"}}
...
func TestLegacyVideoPollErrorsNeverUsePluginFailureRefundThreshold(t *testing.T) {
	for _, tc := range []struct {
		name      string
		adaptor   scriptedPollingAdaptor
		wantError bool
	}{
		{name: "429", adaptor: scriptedPollingAdaptor{statusCode: http.StatusTooManyRequests}},
		{name: "503", adaptor: scriptedPollingAdaptor{statusCode: http.StatusServiceUnavailable}},
		{name: "401", adaptor: scriptedPollingAdaptor{statusCode: http.StatusUnauthorized}},
		{name: "transport failure", adaptor: scriptedPollingAdaptor{fetchErr: io.ErrUnexpectedEOF}, wantError: true},
		{name: "malformed poll result", adaptor: scriptedPollingAdaptor{parseErr: io.ErrUnexpectedEOF}, wantError: true},
	} {
		t.Run(tc.name, func(t *testing.T) {
			truncate(t)
			const userID, channelID, tokenID = 551, 551, 551
			seedUser(t, userID, 10000)
			seedToken(t, tokenID, userID, "sk-legacy-poll", 7000)
			task := makeTask(userID, channelID, 4000, tokenID, BillingSourceWallet, 0)
			task.TaskID = "task_legacy_error"
			task.PrivateData.PollFailures = constant.TaskPollMaxFailures
			require.NoError(t, model.DB.Create(task).Error)
			ch := &model.Channel{Id: channelID, Type: constant.ChannelTypeOpenAIVideo, Key: "test-key"}
			err := updateVideoSingleTask(context.Background(), &tc.adaptor, ch, task.GetUpstreamTaskID(), map[string]*model.Task{task.GetUpstreamTaskID(): task})
			if tc.wantError {
				require.Error(t, err)
			} else {
				require.NoError(t, err)
			...
			assert.Equal(t, 4000, saved.Quota)
			assert.Equal(t, constant.TaskPollMaxFailures, saved.PrivateData.PollFailures)
