...
func TestGetBillingModePreservesConfiguredResolutionPrices(t *testing.T) {
	previousBilling := billingSetting
	previousRatios := ratio_setting.ModelRatio2JSONString()
	previousPrices := ratio_setting.ModelPrice2JSONString()
	previousImagePrices := ratio_setting.ImageResolutionPrice2JSONString()
	t.Cleanup(func() {
		billingSetting = previousBilling
		...
		require.NoError(t, ratio_setting.UpdateImageResolutionPriceByJSONString(previousImagePrices))
	})
	require.NoError(t, ratio_setting.UpdateModelRatioByJSONString(`{}`))
	require.NoError(t, ratio_setting.UpdateModelPriceByJSONString(`{}`))
	billingSetting = BillingSetting{BillingMode: map[string]string{}, TaskBillingPricing: map[string]TaskBillingPriceConfig{}}
	require.NoError(t, ratio_setting.UpdateImageResolutionPriceByJSONString(`{"gpt-image-2":{"1K":0,"2K":0.2,"4K":0.4}}`))
	assert.Equal(t, BillingModeRatio, GetBillingMode("gpt-image-2"), "new built-ins must preserve existing resolution prices")
	require.NoError(t, ratio_setting.UpdateImageResolutionPriceByJSONString(`{}`))
	billingSetting.TaskBillingPricing["gpt-image-2"] = TaskBillingPriceConfig{Mode: BillingModePerRequest, DefaultPrice: common.GetPointer(0.2)}
	assert.Equal(t, BillingModeRatio, GetBillingMode("gpt-image-2"))
	billingSetting.BillingMode["gpt-image-2"] = BillingModeTieredExpr
	assert.Equal(t, BillingModeTieredExpr, GetBillingMode("gpt-image-2"), "an explicit expression selection takes precedence")
}
