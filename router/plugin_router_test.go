...
func TestWebCacheHeadersDoNotLeakOntoPluginRoutes(t *testing.T) {
	...
	assert.Contains(t, fallbackResponse.Header().Get("Cache-Control"), "no-store")
	assert.Equal(t, "test-build", fallbackResponse.Header().Get("Cache-Version"))
}
