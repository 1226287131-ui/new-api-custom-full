...
func SetApiRouter(router *gin.Engine) {
	...
	anonymousRequestBodyLimit := middleware.AnonymousRequestBodyLimit()
	{
		...
		apiRouter.GET("/pricing/task-success-rates", middleware.HeaderNavModuleAuth("pricing"), controller.GetPricingTaskSuccessRates)
		...
		apiRouter.POST("/verify/email/send", middleware.UserAuth(), middleware.CriticalRateLimit(), middleware.UserCriticalRateLimit("security-verification"), middleware.EmailVerificationRateLimit(), middleware.DisableCache(), controller.SendSecurityVerificationEmail)
		...
		userRoute := apiRouter.Group("/user")
		{
			...
			// Login endpoints intentionally do not use the generic critical IP limiter.
			// That limiter counts successful sessions and can block unrelated users
			// sharing a proxy or NAT after only 20 logins in 20 minutes. The API-wide
			// limiter still applies, while registration and account-recovery routes
			// retain the stricter critical limit.
			...
			userRoute.POST("/login/verify", middleware.DisableCache(), anonymousRequestBodyLimit, controller.VerifyLogin)
			userRoute.POST("/login/passkey/begin", middleware.DisableCache(), anonymousRequestBodyLimit, controller.LoginPasskeyBegin)
			userRoute.POST("/login/passkey/finish", middleware.DisableCache(), anonymousRequestBodyLimit, controller.LoginPasskeyFinish)
		...
		dataRoute.GET("/channels/today", middleware.AdminAuth(), controller.GetTodayChannelUsage)
		...
		groupRoute.Use(middleware.AdminAuth())
		{
			...
			groupRoute.GET("/:group/user-ratios", controller.GetGroupUserRatios)
			groupRoute.PUT("/:group/user-ratios", controller.UpsertGroupUserRatio)
			groupRoute.DELETE("/:group/user-ratios/:user_id", controller.DeleteGroupUserRatio)
		}
