...
func PinTaskPluginEndpoint() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// Existing video channels keep their deployed validation, pricing and
		// payload contract even if an installed plugin advertises the same model.
		// A dedicated Task Plugin channel remains available through an explicit
		// pin or a group without a legacy video channel for this model.
		if preferLegacyVideoChannel(c, claimedModel) {
...
func preferLegacyVideoChannel(c *gin.Context, modelName string) bool {
	if c.Request.Method != http.MethodPost || strings.TrimSpace(modelName) == "" {
		return false
	}
	switch c.Request.URL.Path {
	case "/v1/videos", "/v1/video/generations", "/v1/videos/generations":
	default:
		return false
	}
	constraints := service.GetChannelConstraints(c)
	filter := dto.ChannelFilter{Kind: dto.FilterLegacyVideo}
	if pin, exists, _ := constraints.ResolvedPin(); exists {
		ch, err := model.CacheGetChannel(pin.ChannelId)
		if err != nil || ch == nil || !constant.IsVideoTaskChannelType(ch.Type) {
			return false
		}
		constraints.AddFilter(filter)
		return true
	}
	// Unit-level protocol decoders may run without a gateway database. The
	// production path always has one, or the initialized channel memory cache.
	if !common.MemoryCacheEnabled && model.DB == nil {
		return false
	}
	usingGroup := common.GetContextKeyString(c, constant.ContextKeyUsingGroup)
	groups := model.ParseTokenGroups(usingGroup)
	if usingGroup == "auto" {
	...
	filters = append(filters, filter, dto.ChannelFilter{Kind: dto.FilterRequestPath, RequestPath: c.Request.URL.Path})
	for _, group := range groups {
		ch, err := model.GetRandomSatisfiedChannel(group, modelName, 0, filters)
		if err == nil && ch != nil {
			constraints.AddFilter(filter)
			return true
		}
