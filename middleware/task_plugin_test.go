...
func TestVideoEndpointKeepsLegacyChannelsWithoutRunningPluginDecoder(t *testing.T) {
	...
	require.NoError(t, setting.UpdateUserUsableGroupsByJSONString(`{"default":"Default","plugin-only":"Plugin"}`))
	require.NoError(t, ratio_setting.UpdateGroupRatioByJSONString(`{"default":1,"plugin-only":1}`))
...
export const meta = {apiVersion:1,key:"video-collision",name:"Video collision",version:"1.0.0",author:{name:"Test"},models:["collision-video"],fetchMode:"per_task",protocols:["openai_video"]};
export function buildSubmitRequest(){return {url:"https://example.com/video"};}
export function parseSubmitResponse(){return {taskId:"job"};}
...
export function buildContentRequest(){return {url:"https://example.com/content"};}
export const protocols = {openai_video:{
 decodeRequest:function(ctx){if(Number(ctx.body.value.seconds)>15)throw new Error("plugin duration limit");return {kind:"submit",model:ctx.model,requestBody:ctx.body.value};},
 render:function(ctx,task){return task.data;}
}};
`, jsplugin.Options{})
	require.NoError(t, err)
	t.Cleanup(func() { require.NoError(t, jsplugin.DefaultRegistry.Unregister("video-collision")) })
	legacy := &model.Channel{Id: 901, Type: constant.ChannelTypeOpenAIVideo, Name: "legacy", Key: "test-legacy", Status: common.ChannelStatusEnabled, Group: "default", Models: "collision-video"}
	pluginSetting := `{"task_plugin_key":"video-collision"}`
	plugin := &model.Channel{Id: 902, Type: constant.ChannelTypeTaskPlugin, Name: "plugin", Key: "test-plugin", Status: common.ChannelStatusEnabled, Group: "default,plugin-only", Models: "collision-video", Setting: &pluginSetting}
	require.NoError(t, model.DB.Create(legacy).Error)
	require.NoError(t, model.DB.Create(plugin).Error)
	require.NoError(t, model.DB.Create(&[]model.Ability{
		{Group: "default", Model: "collision-video", ChannelId: legacy.Id, Enabled: true},
		{Group: "default", Model: "collision-video", ChannelId: plugin.Id, Enabled: true},
		{Group: "plugin-only", Model: "collision-video", ChannelId: plugin.Id, Enabled: true},
	}).Error)
	model.InitChannelCache()
	for _, tc := range []struct {
		name, group, seconds    string
		pin                     int
		wantChannel, wantStatus int
	}{
		{name: "legacy survives plugin duration limit", group: "default", seconds: "30", wantChannel: 901, wantStatus: 204},
		{name: "multiple authorized groups retain legacy", group: "plugin-only,default", seconds: "30", wantChannel: 901, wantStatus: 204},
		{name: "authorized auto group retains legacy", group: "auto", seconds: "30", wantChannel: 901, wantStatus: 204},
		{name: "explicit legacy pin retains legacy", group: "default", seconds: "30", pin: 901, wantChannel: 901, wantStatus: 204},
		{name: "explicit plugin pin uses decoder", group: "default", seconds: "30", pin: 902, wantStatus: 400},
		{name: "foreign legacy group cannot capture request", group: "plugin-only", seconds: "30", wantStatus: 400},
		{name: "plugin remains available", group: "plugin-only", seconds: "10", wantChannel: 902, wantStatus: 204},
	} {
		t.Run(tc.name, func(t *testing.T) {
			...
			router.POST("/v1/videos", func(c *gin.Context) {
				...
				if tc.group == "auto" {
					common.SetContextKey(c, constant.ContextKeyTokenAutoGroups, []string{"default"})
				}
				if tc.pin > 0 {
					service.GetChannelConstraints(c).AddPin(dto.ChannelPin{ChannelId: tc.pin, Source: dto.PinSourceToken, Rank: dto.PinRankToken, RetryMode: dto.PinRetrySingleAttempt})
				}
				c.Next()
				...
				if tc.wantChannel == 901 {
					_, claimed := c.Get(jsplugin.ContextKeyPinnedEndpoint)
					assert.False(t, claimed)
					var body map[string]any
					require.NoError(t, common.UnmarshalBodyReusable(c, &body))
					assert.Equal(t, tc.seconds, body["seconds"])
				}
				c.Status(http.StatusNoContent)
			})
			request := httptest.NewRequest(http.MethodPost, "/v1/videos", strings.NewReader(fmt.Sprintf(`{"model":"collision-video","prompt":"test","seconds":%q}`, tc.seconds)))
			...
			if tc.wantStatus == 204 {
				...
				assert.Contains(t, recorder.Body.String(), "plugin duration limit")
			}
		})
	}
}
