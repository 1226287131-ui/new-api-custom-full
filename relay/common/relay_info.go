...
type RelayInfo struct {
	...
	TaskBillingPrice *TaskBillingPriceSelection
...
type TaskRelayInfo struct {
	Action       string
	OriginTaskID string
	// SubmissionContext bounds a native video's upstream acceptance/read while
	// allowing it to finish after the client disconnects. Validation still uses
	// the client context, and the transport checks it before sending any work.
	SubmissionContext context.Context `json:"-"`
...
// TaskBillingPriceSelection freezes the configured unit price and resolution
// selected before a task reserves quota.
...
type TaskSubmitReq struct {
	...
	// BillingResolution is an internal, adaptor-selected pricing tier.
