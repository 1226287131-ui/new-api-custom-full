...
// taskSubmitResponseSucceeded treats the complete HTTP 2xx range as a
// successful asynchronous task submission. Providers commonly return 201 or
// 202 after accepting a job, before it has completed.
func taskSubmitResponseSucceeded(statusCode int) bool {
...
func RelayTaskSubmit(c *gin.Context, info *relaycommon.RelayInfo) (*TaskSubmitResult, *dto.TaskError) {
	...
	if useTiered {
		...
		scheduledDiscount := billing_setting.ScheduledDiscountMultiplier(modelName, time.Now())
		quotaBeforeGroup := cost * common.QuotaPerUnit * scheduledDiscount
		...
		if scheduledDiscount != 1 {
			priceData.AddOtherRatio(billing_setting.ScheduledDiscountRatioKey, scheduledDiscount)
		}
		info.TieredBillingSnapshot = &billingexpr.BillingSnapshot{BillingMode: billing_setting.BillingModeTieredExpr, ModelName: modelName, ExprString: exprStr, ExprHash: billingexpr.ExprHashString(exprStr), GroupRatio: groupRatioInfo.GroupRatio, PriceMultiplier: scheduledDiscount, EstimatedQuotaBeforeGroup: quotaBeforeGroup, EstimatedQuotaAfterGroup: quota, EstimatedTier: trace.MatchedTier, QuotaPerUnit: common.QuotaPerUnit, ExprVersion: billingexpr.ExprVersion(exprStr), TaskUsageBilling: true, UsageFacts: facts}
	} else {
	...
	if !taskSubmitResponseSucceeded(resp.StatusCode) {
	...
	} else if taskBillingMode == billing_setting.BillingModePerSecond {
...
func videoFetchByIDRespBodyBuilder(c *gin.Context) (respBody []byte, taskResp *dto.TaskError) {
	...
	legacyTask := originTask.PrivateData.Execution == nil || originTask.PrivateData.Execution.TaskPlugin == nil
...
func tryRealtimeFetch(task *model.Task, isOpenAIVideoAPI bool) []byte {
	...
	localVideoURL := ""
	if constant.IsVideoTaskChannelType(channelModel.Type) && task.Status == model.TaskStatusSuccess {
		videoSourceURL := strings.TrimSpace(ti.Url)
		if videoSourceURL == "" {
			videoSourceURL = strings.TrimSpace(ti.RemoteUrl)
		}
		if videoSourceURL == "" {
