...
func ModelPriceHelper(c *gin.Context, info *relaycommon.RelayInfo, promptTokens int, meta *types.TokenCountMeta) (hosttypes.PriceData, error) {
	...
	scheduledDiscount := billing_setting.ScheduledDiscountMultiplier(info.OriginModelName, time.Now())
	...
	_, useImageResolutionPrice := ratio_setting.GetImageResolutionPrice(billingModelName)
	if useImageResolutionPrice {
		...
		resolutionPrices, _ := ratio_setting.GetImageResolutionPrice(billingModelName)
	...
	if !usePrice {
		...
		ratio := modelRatio * groupRatioInfo.GroupRatio * scheduledDiscount
...
func ModelPriceHelperPerCall(c *gin.Context, info *relaycommon.RelayInfo) (hosttypes.PriceData, error) {
	...
	if usePrice {
		var err error
		quota, err = common.QuotaFromFloatStrict(modelPrice * common.QuotaPerUnit * groupRatioInfo.GroupRatio * scheduledDiscount)
		if err != nil {
			return hosttypes.PriceData{}, err
		}
		if !operation_setting.GetQuotaSetting().EnableFreeModelPreConsume {
		...
		// 按量计费：以模型倍率的一半作为预扣额度
		var err error
		quota, err = common.QuotaFromFloatStrict(modelRatio / 2 * common.QuotaPerUnit * groupRatioInfo.GroupRatio * scheduledDiscount)
		if err != nil {
			return hosttypes.PriceData{}, err
		}
		modelPrice = -1
...
func HasPriceOrRatioEntry(name string) bool {
	...
	if _, ok := ratio_setting.GetImageResolutionPrice(formatted); ok {
		return true
	}
	if _, ok := billing_setting.GetTaskBillingPriceConfig(formatted); ok {
		return true
	}
...
func modelPriceHelperTiered(c *gin.Context, info *relaycommon.RelayInfo, billingModelName string, promptTokens int, meta *types.TokenCountMeta, groupRatioInfo hosttypes.GroupRatioInfo) (hosttypes.PriceData, error) {
	...
	// The discount is frozen in the snapshot so a request that crosses the end
	// of its time window settles at the same price it was accepted with.
	scheduledDiscount := billing_setting.ScheduledDiscountMultiplier(info.OriginModelName, time.Now())
	quotaBeforeGroup := rawCost / 1_000_000 * common.QuotaPerUnit * scheduledDiscount
	...
	snapshot := &billingexpr.BillingSnapshot{
		...
		PriceMultiplier:           scheduledDiscount,
	}
