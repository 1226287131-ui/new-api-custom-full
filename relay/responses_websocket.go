...
func (s *responsesWSSession) restoreConnectionContext(c *gin.Context, model string) *types.NewAPIError {
	...
	if previous, ok := s.lockedContext[appconstant.ContextKeyChannelSetting].(dto.ChannelSettings); ok && service.ResolveChannelProxy(previous) != service.ResolveChannelProxy(channel.GetSetting()) {
...
func dialResponsesWebSocketUpstream(c *gin.Context, adaptor relaychannel.Adaptor, info *relaycommon.RelayInfo) (*websocket.Conn, *types.NewAPIError) {
	...
	if proxy := service.ResolveChannelProxy(info.ChannelSetting); proxy != "" {
		proxyURL, _, proxyErr := common.ParseProxyURLRuntime(proxy)
		if proxyErr != nil {
