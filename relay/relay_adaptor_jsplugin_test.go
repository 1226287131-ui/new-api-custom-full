...
func TestGetTaskAdaptorPreservesLegacyNumericPlatforms(t *testing.T) {
	...
	for _, platform := range platforms {
		...
		assert.Equal(t, platform == constant.TaskPlatformSuno, isJS, "historical numeric platform %s must preserve its deployed adaptor", platform)
	}
}
...
func TestGetTaskAdaptorForRequestPreservesLegacyProvider(t *testing.T) {
	c, _ := gin.CreateTestContext(httptest.NewRecorder())
	...
	_, exists := c.Get(pluginruntime.ContextKeyPinnedPlugin)
	assert.False(t, exists, "legacy requests must not be silently migrated to a plugin")
