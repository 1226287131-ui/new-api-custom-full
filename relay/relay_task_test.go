...
func TestLegacyVideoSubmitWaitsForDurablePublication(t *testing.T) {
	for _, tc := range []struct {
		name      string
		body      string
		wantError bool
	}{
		{name: "accepted video", body: `{"id":"upstream-private-id","status":"queued"}`},
		{name: "accepted malformed reply cannot retry", body: `{"status":"queued"}`, wantError: true},
	} {
		t.Run(tc.name, func(t *testing.T) {
			recorder := httptest.NewRecorder()
			c, _ := gin.CreateTestContext(recorder)
			c.Request = httptest.NewRequest(http.MethodPost, "/v1/videos", nil)
			c.Set("task_request", relaycommon.TaskSubmitReq{Seconds: "30", Size: "1920x1080"})
			info := &relaycommon.RelayInfo{OriginModelName: "customer-video", TaskRelayInfo: &relaycommon.TaskRelayInfo{PublicTaskID: "task_public"}}
			adaptor := GetTaskAdaptor("60")
			require.NotNil(t, adaptor)
			result, taskErr := adaptor.ParseResponse(c, &http.Response{StatusCode: http.StatusAccepted, Body: io.NopCloser(strings.NewReader(tc.body))}, info)
			assert.Empty(t, recorder.Body.String(), "parsing must not send success before persistence")
			assert.False(t, c.Writer.Written())
			if tc.wantError {
				require.NotNil(t, taskErr)
				assert.True(t, taskErr.NoRetry, "an accepted upstream job must not be submitted twice")
				return
			}
			require.Nil(t, taskErr)
			require.NotNil(t, result)
			assert.Equal(t, "upstream-private-id", result.UpstreamTaskID)
			body, err := common.Marshal(result.ClientResponse)
			require.NoError(t, err)
			assert.Contains(t, string(body), "task_public")
			assert.Contains(t, string(body), `"seconds":"30"`)
			assert.NotContains(t, string(body), "upstream-private-id")
		})
	}
}

func TestLegacyVideoPollUsesSelectedEndpointAndPrivateID(t *testing.T) {
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		assert.Equal(t, "/v1/video/generations/provider-task", r.URL.Path)
	...
	adaptor := GetTaskAdaptor("60")
	require.NotNil(t, adaptor)
	adaptor.Init(&relaycommon.RelayInfo{ChannelMeta: &relaycommon.ChannelMeta{ChannelBaseUrl: upstream.URL, ChannelSetting: relaydto.ChannelSettings{OpenAIVideoEndpoint: "/v1/video/generations"}}})
	task := &model.Task{TaskID: "task_public", PrivateData: model.TaskPrivateData{UpstreamTaskID: "provider-task"}}
	response, err := adaptor.FetchTask(upstream.URL, "test-key", task, "")
	require.NoError(t, err)
