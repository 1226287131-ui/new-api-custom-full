...
> & {
  ...
  ImageResolutionPrice?: ImageResolutionPrices
  'billing_setting.task_billing_pricing'?: TaskBillingPriceConfig
  'billing_setting.scheduled_discount'?: ScheduledDiscountConfig
}
...
export function modelPricingDisplay(
  ...
  return {
    ...
    image_resolution_prices: values.ImageResolutionPrice,
    task_billing_pricing: values['billing_setting.task_billing_pricing'],
    scheduled_discount: values['billing_setting.scheduled_discount'],
  }
}
...
export function pricingOptions(
  ...
  return Object.fromEntries(
    PRICING_KEYS.map((key) => {
      ...
      if (key === 'billing_setting.task_billing_pricing') {
        value ??= values.TaskBillingPricing
      }
      if (key === 'billing_setting.scheduled_discount') {
        value ??= values.ScheduledDiscount
      }
...
export function pricingRows(options: PricingOptions): ModelPricingSnapshot[] {
  return buildModelSnapshots({
    ...
    taskBillingPricing: options['billing_setting.task_billing_pricing'],
...
export function pricingFromDraft(data: ModelRatioData): PricingValues {
  ...
  for (const [field, key] of Object.entries(pricingFieldMap)) {
    ...
    if (
      ...
      (data.billingMode === 'per-request' || data.billingMode === 'per-second'
        ? key === 'ModelPrice'
        : key !== 'ModelPrice')
    ) {
      values[key] = number
  ...
  if (data.taskBillingPricing) {
    values['billing_setting.task_billing_pricing'] = JSON.parse(
      data.taskBillingPricing
    ) as TaskBillingPriceConfig
  }
  if (data.scheduledDiscount) {
    values['billing_setting.scheduled_discount'] = JSON.parse(
      data.scheduledDiscount
    ) as ScheduledDiscountConfig
  }
  return values
}
