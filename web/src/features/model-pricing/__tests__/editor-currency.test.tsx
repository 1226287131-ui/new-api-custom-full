...
it('preserves video tiers, zero prices, and scheduled discounts when editing in the site currency', async () => {
  const discount = {
    enabled: true,
    start: '22:00',
    end: '06:00',
  ...
  const editor = renderEditor({
    ...
    taskBillingPricing: JSON.stringify({
      mode: 'per-second',
      resolution_prices: { '720p': 0.2, '4k': 0 },
    }),
    scheduledDiscount: JSON.stringify(discount),
  })
  await selectCurrency('Site currency (CNY)')
  expect(screen.getByRole('textbox', { name: '720p' })).toHaveValue('1.4')
  expect(screen.getByRole('textbox', { name: '4K' })).toHaveValue('0')
  fireEvent.change(screen.getByRole('textbox', { name: '720p' }), {
    target: { value: '2.1' },
  })
  const draft = await commit(editor.ref)
  expect(draft?.billingMode).toBe('per-second')
  expect(JSON.parse(draft?.taskBillingPricing ?? '{}')).toEqual({
    mode: 'per-second',
    resolution_prices: { '720p': 0.3, '4k': 0 },
  })
  expect(JSON.parse(draft?.scheduledDiscount ?? '{}')).toEqual(discount)
})
