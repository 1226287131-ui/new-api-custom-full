...
export function WithdrawalReviewDialog(props: {
  ...
  const disclosure = useMutation({
    mutationFn: (proof: string) => getWithdrawalPayout(props.item.id, proof),
    onSuccess: setPayout,
  })
  const busy =
    verification.isActive || mutation.isPending || disclosure.isPending
  async function review(action: WithdrawalReview['action']) {
    if (busy) return
    const body = { action, reference: reference.trim(), reason: reason.trim() }
        ...
        <div className='space-y-5'>
          ...
          <Button
            ...
            onClick={async () => {
              ...
              if (!proof) return
              disclosure.mutate(proof.proof_token)
            }}
          >
