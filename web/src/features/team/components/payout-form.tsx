...
export function PayoutForm(props: { data: TeamSelf }) {
  ...
  const disabled =
    !props.data.payout_ready || mutation.isPending || verification.isLoading
  return (
      ...
      <form
        onSubmit={form.handleSubmit((body) => {
          void verification.startVerification(
            (proof) => mutation.mutateAsync({ body, proof }),
            { scope: 'team.payout.write', title: t('Confirm payout account') }
          )
        })}
      >
        <FieldGroup>
