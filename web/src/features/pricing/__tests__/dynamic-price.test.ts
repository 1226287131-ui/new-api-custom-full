...
describe('expression price summaries', () => {
  test.each([
    ['2026-09-15T13:59:00Z', '$0.2'],
    ['2026-09-15T14:00:00Z', '$0.16'],
    ['2026-09-15T22:00:00Z', '$0.2'],
  ])(
    'combines task usage, group and recharge prices with the Beijing discount window at %s',
    (at, expected) => {
      const model = pricingModel({
        billing_mode: 'tiered_expr',
        billing_expr: 'tier("seconds", u("seconds") * 0.2)',
      ...
      expect(summary?.primaryEntries[0]?.formatted).toBe(expected)
    }
  )
