...
export function formatTaskUsageUnitPrice(
  ...
  const usdExchangeRate = options.usdExchangeRate ?? 1
  const priceUSD =
    valuePerUnit * groupRatio * (options.scheduledDiscountMultiplier ?? 1)
  const displayPrice = applyRechargeRate(
    priceUSD,
    options.showRechargePrice ?? false,
    priceRate,
...
export function getDynamicPricingSummary(
  ...
  options = {
    ...options,
    scheduledDiscountMultiplier:
      options.scheduledDiscountMultiplier ??
      getScheduledDiscountMultiplier(model, options.now),
  }
